# Anthology Master List | Cheat Sheets <!-- omit from toc -->

This Anthology document contains the master list of various cheat sheats I have built or collected, this file is referenced in each subsequent .md file, so that it can be used to return to the master list.

This template is used to create a standard how to .md document

<a id="TOC"></a>
Table of Contents

- [Cheet Sheet Original](#cheet-sheet-original)
- [Cheet Sheet Collection](#cheet-sheet-collection)
- [Blockquote Manual List using 2 spaces at the end](#blockquote-manual-list-using-2-spaces-at-the-end)
- [Ordered list using numbers](#ordered-list-using-numbers)

## Cheet Sheet Original

[TOC](#TOC)

- [Visual Studio Code](./cheat-sheets/cheat-sheets_visual-studio-code.md)
- [process 2](./cheat-sheets/cheat-sheets_TEMPLATE.md)
- [process 3](./cheat-sheets/cheat-sheets_TEMPLATE.md)

## Cheet Sheet Collection

[TOC](#TOC)
- [process 2](./cheat-sheets/cheat-sheets_TEMPLATE.md)
- [process 3](./cheat-sheets/cheat-sheets_TEMPLATE.md)
- [process 3](./cheat-sheets/cheat-sheets_TEMPLATE.md)

## Blockquote Manual List using 2 spaces at the end

[TOC](#TOC)

A block quote must have text in between the `>` paragraph marks, otherwise, you need two (2) spaces after for a new line.

>[process 1](./cheat-sheets/cheat-sheets_TEMPLATE.md)  
[process 2](./cheat-sheets/cheat-sheets_TEMPLATE.md)  
[process 3](./cheat-sheets/cheat-sheets_TEMPLATE.md)  

## Ordered list using numbers

[TOC](#TOC)

1. [process 1](./cheat-sheets/cheat-sheets_TEMPLATE.md)
2. [process 2](./cheat-sheets/cheat-sheets_TEMPLATE.md)
3. [process 3](./cheat-sheets/cheat-sheets_TEMPLATE.md)
